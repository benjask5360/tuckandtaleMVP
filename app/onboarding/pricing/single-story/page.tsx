...
function SingleStoryPricingContent() {
  ...
  useEffect(() => {
    const initPage = async () => {
      ...
      if (!user) {
        router.push('/auth/login?promo=single-story')
        return
      }
  ...
  const handlePurchase = async () => {
    try {
      ...
      const response = await fetch('/api/stripe/create-story-checkout', {
        ...
        body: JSON.stringify({
          // No storyId - this is for buying a generation credit
          // Indicate this is from onboarding for proper redirect
          fromOnboarding: true,
        }),
      })
  ...
  const getSubtext = () => {
    if (loadingCharacters || characters.length === 0) {
      return 'Create a magical personalized story for your child'
    }
    const names = characters.map(c => c.name)
    if (names.length === 1) {
      return `${names[0]} is ready for their personalized adventure!`
    }
    if (names.length === 2) {
      return `${names[0]} and ${names[1]} are ready for their personalized adventure!`
    }
    const lastIndex = names.length - 1
    return `${names.slice(0, lastIndex).join(', ')}, and ${names[lastIndex]} are ready for their personalized adventure!`
  }
          ...
          <div className="bg-white rounded-3xl shadow-xl p-8 md:p-10">
            ...
            <div className="space-y-3 mb-8">
              <div className="flex items-start gap-3">
                ...
                <span className="text-gray-700">One personalized story featuring your child</span>
              </div>
              <div className="flex items-start gap-3">
                <div className="flex-shrink-0 w-6 h-6 bg-green-100 rounded-full flex items-center justify-center">
                  <Check className="w-4 h-4 text-green-600" />
              ...
              <div className="flex items-start gap-3">
                ...
                <span className="text-gray-700">Choose your adventure theme</span>
              </div>
              <div className="flex items-start gap-3">
                <div className="flex-shrink-0 w-6 h-6 bg-green-100 rounded-full flex items-center justify-center">
                  <Check className="w-4 h-4 text-green-600" />
                </div>
                <span className="text-gray-700">Saved to your library</span>
              </div>
            </div>
            ...
            >
              {processingCheckout ? (
                ...
                <>
                  Get Your Story - {DISPLAY_PRICES.SINGLE_STORY}
...
export default function SingleStoryPricing() {
  return (
    <Suspense fallback={<LoadingFallback />}>
      <SingleStoryPricingContent />
    </Suspense>
  )
}
