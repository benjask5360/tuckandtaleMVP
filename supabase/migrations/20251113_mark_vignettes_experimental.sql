...
COMMENT ON COLUMN public.content.vignette_helper_prompt IS '[EXPERIMENTAL] OpenAI prompt for generating visual scene descriptions (Call #1 in vignette generation pipeline)';

COMMENT ON COLUMN public.content.vignette_prompt IS '[EXPERIMENTAL] Leonardo prompt for generating panoramic storyboard image (Call #2 in vignette generation pipeline)';
